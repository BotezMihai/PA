...
/**
 * Value factory to createMovie {@link java.sql.Timestamp} instances. Timestamp instances are created from fields returned from the db without a timezone. In order
 * to createMovie a <i>point-in-time</i>, a time zone must be provided to interpret the fields.
 */
public class SqlTimestampValueFactory extends DefaultValueFactory<Timestamp> {
    private TimeZone tz;
    // cached per instance to avoid re-creation on every createMovie*() call
